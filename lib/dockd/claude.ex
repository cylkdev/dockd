...
#       --verbose …` via `Docker.exec_instance/3` and returns a lazy
...
#   `Dockd` itself is concerned with provisioning a instance and exposing
#   `Dockd.shell_command/2` (one-shot) and `Dockd.open_shell/1` (persistent)
#   for programmatic exec. `Dockd.Claude` is a higher-level counterpart
#   specific to the Claude CLI: it makes individual Q&A calls rather than
#   running arbitrary commands.

#   ## Example

#       {:ok, instance} = Dockd.prepare_package("claude_code")
#       {:ok, response} = Dockd.Claude.ask(instance, "what is 2+2")
#       response["result"]
#       #=> "4"
...
#           continue: boolean(),
#           resume: binary() | true,
#           no_instance_persistence: boolean(),
...
#   @doc """
#   Asks Claude a single question inside `instance` and returns the parsed
#   JSON result.
...
#   """
#   @spec ask(Instance.t(), binary(), ask_opts()) :: {:ok, response()} | {:error, term()}
#   def ask(%Instance{} = instance, prompt, opts \\ []) when is_binary(prompt) and is_list(opts) do
#     with {:ok, argv} <- to_argv(opts, :sync) do
#       argv = argv ++ [prompt]
#       exec_opts = build_exec_opts(instance, opts)

#       case Docker.exec_run_with_status(instance.container_id, argv, exec_opts) do
...
#   `opts`, opens an exec instance via `Docker.exec_instance/3`, and wraps
#   the socket in `Stream.resource/3`. Each line of stdout is decoded
#   with `JSON.decode!/1` and emitted as a map.
...
#   Each line is decoded with `JSON.decode!/1`; a malformed line will
#   raise `JSON.DecodeError` from inside the stream - the CLI is
#   contractually expected to emit valid newline-delimited JSON.
#   """
#   @spec ask_stream(Instance.t(), binary(), stream_opts()) ::
#           {:ok, Enumerable.t()} | {:error, term()}
#   def ask_stream(%Instance{} = instance, prompt, opts \\ [])
#       when is_binary(prompt) and is_list(opts) do
#     with {:ok, argv} <- to_argv(opts, :stream) do
#       argv = argv ++ [prompt]

#       exec_opts =
#         instance.docker_options
#         |> Keyword.put(:attach_stdin, false)
#         |> Keyword.put(:tty, false)

#       timeout = Keyword.get(opts, :timeout, @default_stream_chunk_timeout)

#       case Docker.exec_instance(instance.container_id, argv, exec_opts) do
#         {:ok, exec_instance} ->
#           {:ok, build_event_stream(exec_instance, timeout)}
...
#   defp opt_to_argv({:instance_id, uuid}, _mode) when is_binary(uuid),
#     do: ["--instance-id", uuid]
...
#   defp opt_to_argv({:no_instance_persistence, true}, _mode), do: ["--no-instance-persistence"]
#   defp opt_to_argv({:no_instance_persistence, _}, _mode), do: []
...
#   defp build_exec_opts(%Instance{docker_options: docker_options}, opts) do
...
#   # stateful - recv/3 returns an updated instance each call) plus a
#   # leftover-buffer string for any partial line read at the previous
#   # boundary.
#   defp build_event_stream(exec_instance, timeout) do
#     Stream.resource(
#       fn -> {exec_instance, ""} end,
...
#   defp next_events({instance, leftover}, timeout) do
#     case Session.recv(instance, {:until, "\n"}, timeout: timeout) do
#       {:ok, chunk, instance2} ->
...
#         {events, {instance2, new_leftover}}

#       {:error, :closed_before_delimiter, instance2} ->
#         flush_leftover(leftover, instance2)

#       {:error, :closed, instance2} ->
#         flush_leftover(leftover, instance2)

#       {:error, _reason, instance2} ->
#         # Surface as halt; after_fun will close.
#         {:halt, {instance2, leftover}}
#     end
#   end
...
#   defp flush_leftover("", instance), do: {:halt, {instance, ""}}

#   defp flush_leftover(leftover, instance) do
#     case JSON.decode(leftover) do
#       # Emit the final event, then on the next call we'll see {leftover: ""}
#       # plus a closed instance - recv returns {:error, :closed, _}, which
#       # will halt cleanly.
#       {:ok, event} -> {[event], {instance, ""}}
#       {:error, _} -> {:halt, {instance, ""}}
#     end
#   end
# end
