defmodule Dockd.Workspace do
  @moduledoc """
  The state of a single Docker workspace tracked by `Dockd`.

  A workspace bundles together everything a caller needs to operate on a prepared container:
  ...
  ## Responsibilities
    ...
    - Carry forward the Docker connection options used to create the container so later
      operations on the same workspace reach the same daemon

  ## Examples

      iex> workspace = Dockd.Workspace.new(%{container_name: "dockd-1", shell: "/bin/bash"})
      iex> workspace.shell_command
      "docker exec -it dockd-1 /bin/bash"

      iex> workspace = Dockd.Workspace.new(%{container_name: "dockd-2", shell: "/bin/sh"})
      iex> updated = Dockd.Workspace.put(workspace, %{container_id: "abc123"})
  ...
  #   A %Workspace{} represents a Docker workspace at one point in its lifecycle.
  ...
  #   Base case: a workspace with container_id == nil and step_results == [] represents
  ...
  #
  #   %Workspace{name: "dockd-1", shell: "/bin/sh", id: nil}
  #          |                                            |
  #          | put(%{container_id: "abc"})                |
  #          v                                            v
  #   %Workspace{name: "dockd-1", shell: "/bin/sh", id: "abc",
  ...
  def new(attrs) when is_map(attrs) do
    workspace = struct(__MODULE__, Map.to_list(attrs))
    %{workspace | shell_command: shell_command(workspace)}
  end

  @spec put(t(), map()) :: t()
  def put(%__MODULE__{} = workspace, attrs) when is_map(attrs) do
    workspace
    |> Map.merge(attrs)
