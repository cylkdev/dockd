defmodule Dockd.Git do
  ...
  cloned into a fresh per-call host tempdir — shallow (`depth: 1`) by default,
  with `.git` stripped unless `history: true` — and then all repos are forwarded
  to `Dockd.FileCopy.copy_files/3` as a single batched upload into
  `container_id`. The host tempdir is always removed afterwards.

  `docker_options` carries the Docker connection options used for the upload.
