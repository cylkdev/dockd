defmodule Mix.Tasks.Dockd.Instance.Run do
  ...
  JSON package on disk, or bundled package by name - calls into `Dockd.apply/2` (or
  ...
  Start a container from a bundled package (resolved to `priv/packages/<NAME>.json`):
      ...
      ANTHROPIC_API_KEY=<your-key> mix dockd.instance.run --preset claude_code_live_instance

  Suppress all output except the bare attach command - useful when scripting:

      mix dockd.instance.run --preset claude_code_live_instance --short
  ...
  task ends, so closing the terminal doesn't tear it down:
      ...
      mix dockd.instance.run --preset claude_code_live_instance --detached
  ...
  for cleanup, then returns. Combine with `--short` for scriptable single-line output:
      ...
      eval "$(mix dockd.instance.run --preset claude_code_live_instance --short --detached)"
  ...
  one - pairs well with `--detached`:
      ...
      mix dockd.instance.run --preset claude_code_live_instance --detached --name claude-work
      # ...later, from anywhere:
