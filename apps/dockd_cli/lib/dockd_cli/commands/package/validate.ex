defmodule DockdCLI.Commands.Package.Validate do
  ...
  def run_json(args, opts) do
    case Map.get(args, :name) do
      ref when is_binary(ref) and ref != "" -> validate_one_json(ref, opts)
      _ -> validate_all_json(opts)
    end
  end
  ...
  defp validate_all_json(opts) do
    results = opts |> Packages.list() |> Enum.map(&validate_package(&1, opts))
    Output.json(Enum.map(results, &result_json/1))
    if Enum.any?(results, &match?({_, {:error, _}}, &1)), do: exit_failure(), else: :ok
  end

  defp validate_one_json(ref, opts) do
    path = Packages.resolve_path(ref, opts)
    name = ref |> Path.basename() |> Path.rootname(".json")
    result = {name, validate_path(path)}
    Output.json(result_json(result))
    if match?({_, {:error, _}}, result), do: exit_failure(), else: :ok
  end
  ...
  defp result_json({name, {:ok, _spec}}), do: %{name: name, valid: true}

  defp result_json({name, {:error, %Error{phase: phase, message: message}}}),
    do: %{name: name, valid: false, phase: phase, message: message}
