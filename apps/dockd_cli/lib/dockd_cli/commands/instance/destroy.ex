defmodule DockdCLI.Commands.Instance.Destroy do
  ...
  def run_json(%{all: true} = args, opts) do
    if Map.get(args, :force, false) do
      destroy_all_json(opts)
    else
      {:error, "--all requires --force with --json"}
    end
  end

  def run_json(%{name: name}, opts) when is_binary(name) and name != "" do
    case Dockd.destroy(name, opts) do
      :ok -> Output.json(Json.action(name, "destroyed"))
      {:error, err} -> {:error, err}
    end
  end

  def run_json(_args, _opts), do: {:error, "Usage: dockd instance destroy NAME | --all [--force]"}
  ...
  defp destroy_all_json(opts) do
    case Dockd.list(opts) do
      {:ok, []} ->
        Output.json([])

      {:ok, instances} ->
        results = Enum.map(instances, &destroy_one_json(&1, opts))
        Output.json(Enum.map(results, &elem(&1, 1)))
        failures = Enum.count(results, &(elem(&1, 0) == :error))

        if failures > 0,
          do: {:error, "#{failures} of #{length(instances)} instance(s) failed to destroy"},
          else: :ok
  ...
  defp destroy_one_json(instance, opts) do
    name = Instance.short_name(instance)

    case Dockd.destroy(instance, opts) do
      :ok ->
        {:ok, %{name: name, action: "destroyed", status: "ok"}}

      {:error, err} ->
        {:error,
         %{name: name, action: "destroyed", status: "error", error: Exception.message(err)}}
    end
  end
end
