defmodule DockdCLI.Commands.Instance.Inspect do
  ...
  def run_json(%{name: name}, opts) when is_binary(name) and name != "",
    do: render_json(Dockd.inspect(name, opts))

  def run_json(_args, _opts), do: {:error, "Usage: dockd instance inspect NAME"}

  @doc false
  def render_json({:ok, map}), do: Output.json(map)
