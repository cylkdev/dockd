defmodule Dockd.Ssh.DockerDialStdio do
  ...
  decoupled capabilities:
    ...
    * **Generate** the rendered template to disk — see the
      `mix dockd.ssh.dial_stdio_script.generate` task.
  ...
  def template_path do
    Path.join([:code.priv_dir(:dockd_ssh), "eex", @template_filename])
  end
  ...
  This is the destination used by `mix dockd.ssh.install_dial_stdio_script`
  when the caller does not pass `--remote-path`.
