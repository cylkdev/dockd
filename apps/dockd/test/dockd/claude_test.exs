...
#   # generated claude_code package). Without creds the container starts
...
#   # Probe a freshly-prepared instance to confirm `claude` can actually
...
#   defp claude_authenticated?(instance) do
#     case Claude.ask(instance, "ping", timeout: 30_000) do
...
#     test "instance_id emits --instance-id <uuid>" do
#       assert {:ok, argv} = Claude.to_argv([instance_id: "abc-uuid"], :sync)
#       i = Enum.find_index(argv, &(&1 == "--instance-id"))
...
#     test "no_instance_persistence: true emits the flag" do
#       assert {:ok, argv} = Claude.to_argv([no_instance_persistence: true], :sync)
#       assert "--no-instance-persistence" in argv
#     end
...
#       instance = %Dockd.Session{container_id: "no-such-container", docker_options: []}

#       assert {:error, %{reason: :unknown_opt, key: :garbage}} =
#                Claude.ask(instance, "x", garbage: 1)
#     end
#   end
...
#       assert {:error, %{reason: :unknown_opt, key: :garbage}} =
#                Claude.ask_stream(instance, "x", garbage: 1)
#     end
#   end
...
#         :ok
#       else
#         assert {:ok, instance} = Dockd.prepare_package("claude_code")
#         on_exit(fn -> Dockd.destroy(instance) end)

#         if claude_authenticated?(instance) do
#           assert {:ok, response} =
#                    Claude.ask(instance, "what is 2+2, answer with just the number",
#                      timeout: 120_000
#                    )
...
#         # instance will also produce - that's still a valid error shape
#         # for the contract under test, so we don't gate on auth here.
#         assert {:error, err} =
#                  Claude.ask(instance, "hi",
...
#         if claude_authenticated?(instance) do
#           assert {:ok, stream} =
#                    Claude.ask_stream(instance, "name two prime numbers", timeout: 120_000)
...
#     @tag :network
#     @tag timeout: 180_000
#     test "early stream halt closes the underlying docker instance" do
#       unless creds_present?() do
#         IO.puts("[skip] no claude credentials in host env")
#         :ok
#       else
...
#         if claude_authenticated?(instance) do
#           assert {:ok, stream} =
#                    Claude.ask_stream(instance, "list ten primes", timeout: 120_000)
